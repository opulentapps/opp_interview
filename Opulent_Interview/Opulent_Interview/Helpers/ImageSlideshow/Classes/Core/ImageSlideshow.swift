...
public enum PageControlPosition {
    ...
    var bottomPadding: CGFloat {
        switch self {
        case .hidden, .insideScrollView, .userCustom:
            return 0.0
        case .underScrollView:
            return 30.0
...
open class ImageSlideshow: UIView {
    ...
    open var pageControlFrameCustom : CGRect?
    ...
    override open func layoutSubviews() {
        ...
        if case .hidden = self.pageControlPosition {
            ...
            let pFrame = CGRect(x: 0, y: 0, width: frame.size.width, height: 10)
            if case .userCustom = self.pageControlPosition {
                let uframe = pageControlFrameCustom == nil ? pFrame : pageControlFrameCustom!
                pageControl.frame = uframe
                pageControl.center = CGPoint(x: uframe.x + uframe.width/2, y: uframe.y + uframe.height/2)
            }
            else{
                pageControl.frame = pFrame
